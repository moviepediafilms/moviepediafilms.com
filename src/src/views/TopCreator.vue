  ...
  <base-layout>
    <div class="q-ma-sm text-center">
      ...
      <q-card flat class="q-mt-md" v-if="contests.length > 0">
        <q-tabs v-model="tab" dense class="white" indicator-color="primary" inline-label outside-arrows mobile-arrows>
          <q-tab :name="contest.name" :label="contest.name" v-for="contest in contests" :key="contest.id" />
        </q-tabs>
        <q-tab-panels v-model="tab" animated>
          <q-tab-panel :name="contest.name" v-for="contest in contests" :key="contest.id">
            <creator-paginated :contest="contest" @click="on_profile_select" :key="contest.id" />
          </q-tab-panel>
        </q-tab-panels>
      </q-card>
      <empty-state title="Nothing to show here."
        desc="Check this space later to find out who's on top of our leaderboards" image="/img/empty/15.svg" v-else />
    </div>
  </base-layout>
</template>
<script>
