  ...
  <base-layout>
    <div class="q-pa-md">
      ...
      <div v-else>
          ...
          <div class="col-12" v-for="movie in submissions" :key="movie.id">
            <div class="row">
              <q-card class="col-3 q-ma-md">
                <movie-image class="col-4" :title="movie.title" :state="movie.state" :show-state="true"
                  :poster="movie.poster" />
              </q-card>
              <div class="col">
                <div>
                  ...
                  <div class="q-mt-lg" :key="order.id" v-for="order in movie.orders">
                    ...
                    <q-btn size="sm" label="Pay" color="primary" class="q-mt-xs" text-color="dark"
                      :to="{ name: 'submit', params: { movie_id: movie.id } }"
                      v-if="has_active_package(order) && is_not_paid(order)" />
                    <q-btn size="sm" class="q-mt-md" label="Select Package" color="primary" text-color="dark"
                      :to="{ name: 'submit', params: { movie_id: movie.id } }" v-if="!order.order_id" />
                  </div>
                </div>
              </div>
            </div>
        ...
        <empty-state title="Nothing here to see" desc="Submit movie now!" icon="mdi-emoticon-sad" v-else />
      </div>
    </div>
  </base-layout>
</template>
